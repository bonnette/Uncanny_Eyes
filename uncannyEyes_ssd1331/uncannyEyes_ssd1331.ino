...
// Modified by Larry Bonnette to use the Adafruit 16-bit Color 0.96" OLED Breakout Board w/microSD holder (SSD1331)
// with the Teensey 3.2
//--------------------------------------------------------------------------

#include <SPI.h>
#include <Adafruit_GFX.h>
#include <Adafruit_SSD1331.h>
#include "graphics/defaultEye1.h" 
...
//  {  9, 0, 0 }, // LEFT EYE display-select and wink pins, no rotation
  { 10, -1, 0 }, // RIGHT EYE display-select and wink pins, no rotation
};

//#include "config.h"     // ****** CONFIGURATION IS DONE IN HERE ******

//  typedef Adafruit_SSD1331 displayType; // Using OLED display(s)
...
#define SCRNH 64
#define SCRNW 96
#define AUTOBLINK // if defined the eye autoblinks
#define TRACKING  // if defined the upper and lower eye lids track the eyeball movement
#define  BLACK  0x0000
#define  BLUE   0x001F
#define RED     0xF800
#define GREEN   0x07E0
...
pinMode(cs, OUTPUT);
digitalWrite(cs, HIGH); // Deselect them all
      

#if defined(DISPLAY_RESET) && (rst>= 0)
  ...
  // microcontroller reset, in which case rst should be set
...
//Test pattern
  uint8_t w,h;
  display.setAddrWindow(0, 0, SCRNW, SCRNH);
...
void drawEye( // Renders one eye.  Inputs must be pre-clipped & valid.
  ...
  digitalWrite(cs, LOW);                // Re-chip-select
  digitalWrite(dc, HIGH);                      // Data mode
  // Now just issue raw 16-bit values for every pixel...

  scleraXsave = scleraX; // Save initial X value to reset on each line
  irisY       = scleraY - (SCLERA_HEIGHT - IRIS_HEIGHT) / 2;
  for(screenY=0; screenY<SCRNH; screenY++, scleraY++, irisY++) {
    scleraX = scleraXsave;
    irisX   = scleraXsave - (SCLERA_WIDTH - IRIS_WIDTH) / 2;
    for(screenX=0; screenX<SCRNW; screenX++, scleraX++, irisX++) {
...
void frame( // Process motion for a single frame of left or right eye
  ...
  eyeX = map(eyeX, 0, 1023, 0, SCLERA_WIDTH  - SCRNW);
  eyeY = map(eyeY, 0, 1023, 0, SCLERA_HEIGHT - SCRNH);
  if(eyeIndex == 1) eyeX = (SCLERA_WIDTH - SCRNW) - eyeX; // Mirrored display
  ...
  if(sampleY < 0) n = 0;
                       ...
                       upper[sampleY][SCRNW - 1 - sampleX]) / 2;
